...
%% ============================================================
%  optimization_comparison：GA / PSO / DE 三种传统算法对比
%  运行本脚本，可生成：
%    1. 设计水头典型工况下的优化前指标；
%    2. 开环导叶控制与 PID 闭环修正的响应对比；
%    3. GA、PSO、DE 优化结果对比；
%    4. 多张论文可用图和 CSV 表格。
%% ============================================================
...
fig = figure('Name', 'fig_01_typical_condition_response', 'Color', 'w', 'Position', [80 60 1450 820], 'Visible', cfg.plot.visible);
...
titles = {'(a) 转速响应','(b) 导叶开度响应','(c) 过机流量响应','(d) 有效水头响应','(e) 蜗壳压力水头响应','(f) 尾水压力水头响应'};
for k = 1:6
    nexttile(tl, k);
    ...
    format_axes('时间 / s', ylabels{k}, titles{k});
end
sgtitle(tl, '设计水头典型工况优化前开机过渡过程响应', 'FontSize', 16, 'FontWeight', 'bold');
save_figure(fig, cfg, 'fig_01_typical_condition_response.png');
end

function plot_pid_speed_guide(out_no, out_pid, p, cfg)
fig = figure('Name', 'fig_02_pid_correction_speed_guide', 'Color', 'w', 'Position', [100 100 1200 480], 'Visible', cfg.plot.visible);
...
yline(p.nr, ':', '额定转速', 'LineWidth', 1.0);
xline(p.guide.t3, ':', 'PID接入', 'LineWidth', 1.0);
format_axes('时间 / s', '转速 / (r/min)', '(a) 转速响应对比');
...
xline(p.guide.t3, ':', 'PID接入', 'LineWidth', 1.0);
format_axes('时间 / s', '导叶开度 / deg', '(b) 导叶开度响应对比');
legend('开环导叶控制', 'PID闭环修正', 'Location', 'best');

sgtitle(tl, '开环导叶控制与PID闭环修正下转速及导叶响应对比', 'FontSize', 15, 'FontWeight', 'bold');
save_figure(fig, cfg, 'fig_02_pid_correction_speed_guide.png');
end

function plot_pid_response(out_no, out_pid, p, cfg)
fig = figure('Name', 'fig_03_pid_correction_response', 'Color', 'w', 'Position', [80 60 1450 820], 'Visible', cfg.plot.visible);
tl = tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
fields = {'n_rpm','y_deg','q_m3s','he_m','casing_pressure_head_m','tail_pressure_head_m'};
ylabels = {'转速 / (r/min)','导叶开度 / deg','流量 / (m^3/s)','有效水头 / m','蜗壳压力水头 / m','尾水压力水头 / m'};
titles = {'(a) 转速响应','(b) 导叶开度响应','(c) 过机流量响应','(d) 有效水头响应','(e) 蜗壳压力水头响应','(f) 尾水压力水头响应'};
...
sgtitle(tl, '开环导叶控制与PID闭环修正下开机过渡过程响应对比', 'FontSize', 16, 'FontWeight', 'bold');
save_figure(fig, cfg, 'fig_03_pid_correction_response.png');
end

function plot_algorithm_response(outs, methods, p, cfg)
fig = figure('Name', 'fig_04_algorithm_response', 'Color', 'w', 'Position', [80 60 1450 820], 'Visible', cfg.plot.visible);
tl = tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
fields = {'n_rpm','y_deg','q_m3s','he_m','casing_pressure_head_m','tail_pressure_head_m'};
ylabels = {'转速 / (r/min)','导叶开度 / deg','流量 / (m^3/s)','有效水头 / m','蜗壳压力水头 / m','尾水压力水头 / m'};
titles = {'(a) 转速响应','(b) 导叶开度响应','(c) 过机流量响应','(d) 有效水头响应','(e) 蜗壳压力水头响应','(f) 尾水压力水头响应'};
...
sgtitle(tl, '不同优化方案开机过渡过程响应对比', 'FontSize', 16, 'FontWeight', 'bold');
save_figure(fig, cfg, 'fig_04_algorithm_response.png');
end

function plot_algorithm_zoom(outs, methods, p, cfg)
fig = figure('Name', 'fig_05_algorithm_zoom', 'Color', 'w', 'Position', [100 100 1200 800], 'Visible', cfg.plot.visible);
tl = tiledlayout(2, 2, 'TileSpacing', 'compact', 'Padding', 'compact');

nexttile(tl, 1);
plot_methods(outs, 'n_rpm', cfg); hold on; yline(p.nr, ':', '额定转速'); xlim(cfg.zoom.speed_xlim); if isfield(cfg.zoom, 'speed_ylim'), ylim(cfg.zoom.speed_ylim); end
if isfield(cfg.zoom, 'speed_ylim'), ylim(cfg.zoom.speed_ylim); end
format_axes('时间 / s', '转速 / (r/min)', '(a) 转速稳定段局部放大'); legend(methods, 'Location', 'best');

nexttile(tl, 2);
plot_methods(outs, 'y_deg', cfg); xlim(cfg.zoom.guide_xlim);
format_axes('时间 / s', '导叶开度 / deg', '(b) 导叶动作局部放大'); legend(methods, 'Location', 'best');

nexttile(tl, 3);
plot_methods(outs, 'casing_pressure_head_m', cfg); xlim(cfg.zoom.pressure_xlim);
format_axes('时间 / s', '蜗壳压力水头 / m', '(c) 蜗壳压力局部放大'); legend(methods, 'Location', 'best');

nexttile(tl, 4);
plot_methods(outs, 'tail_pressure_head_m', cfg); xlim(cfg.zoom.pressure_xlim);
format_axes('时间 / s', '尾水压力水头 / m', '(d) 尾水压力局部放大'); legend(methods, 'Location', 'best');

sgtitle(tl, '不同优化方案关键响应局部放大对比', 'FontSize', 15, 'FontWeight', 'bold');
save_figure(fig, cfg, 'fig_05_algorithm_zoom.png');
end

function plot_pressure_comparison(outs, methods, cfg)
fig = figure('Name', 'fig_06_pressure_comparison', 'Color', 'w', 'Position', [100 100 1200 480], 'Visible', cfg.plot.visible);
tl = tiledlayout(1, 2, 'TileSpacing', 'compact', 'Padding', 'compact');

nexttile(tl, 1);
plot_methods(outs, 'casing_pressure_head_m', cfg);
format_axes('时间 / s', '蜗壳压力水头 / m', '(a) 蜗壳压力水头对比');
legend(methods, 'Location', 'best');

nexttile(tl, 2);
plot_methods(outs, 'tail_pressure_head_m', cfg);
format_axes('时间 / s', '尾水压力水头 / m', '(b) 尾水压力水头对比');
legend(methods, 'Location', 'best');

sgtitle(tl, '不同优化方案压力响应对比', 'FontSize', 15, 'FontWeight', 'bold');
save_figure(fig, cfg, 'fig_06_pressure_comparison.png');
end
...
function format_axes(xlab, ylab, ttl)
grid on;
box on;
xlabel(xlab);
ylabel(ylab);
...
% 这张图不改变仿真结果，只把“优化方案-优化前方案”的差异单独放大，
% 适合论文中说明优化主要改变了哪些响应阶段。
base = outs{1};
fig = figure('Name', 'fig_11_response_difference', 'Color', 'w', 'Position', [100 100 1300 720], 'Visible', cfg.plot.visible);
tl = tiledlayout(2, 2, 'TileSpacing', 'compact', 'Padding', 'compact');

fields = {'n_rpm', 'q_m3s', 'casing_pressure_head_m', 'tail_pressure_head_m'};
ylabels = {'转速差值 / (r/min)', '流量差值 / (m^3/s)', '蜗壳压力差值 / m', '尾水压力差值 / m'};
titles = {'(a) 转速相对优化前差值', '(b) 流量相对优化前差值', '(c) 蜗壳压力相对优化前差值', '(d) 尾水压力相对优化前差值'};
...
sgtitle(tl, '不同优化方案相对优化前响应差值对比', 'FontSize', 15, 'FontWeight', 'bold');
save_figure(fig, cfg, 'fig_11_response_difference.png');
end

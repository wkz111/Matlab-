...
% 低水头绿色、设计水头深灰、高水头蓝色。验证图里不突出优化算法，避免和第四章 PSO 主推图混淆。
...
for i = 1:n_case
    ...
    % prepare_objective_params 会完成一次典型参数仿真并返回响应结果。
...
fig = figure('Name', 'fig_01_three_head_response', 'Color', 'w', 'Position', [80 60 1450 820]);
...
for k = 1:6
    ...
    if k == 1
        ...
        % 三水头模型验证不标注启动稳定时间。
        % 这里验证的是水头变化下模型响应是否合理，不评价各水头下控制性能优劣。
    elseif k == 2
...
sgtitle(tl, '不同水头工况下开机过渡过程响应对比', 'FontSize', 16, 'FontWeight', 'bold');
save_figure(fig, output_dir, 'fig_01_three_head_response.png');
end

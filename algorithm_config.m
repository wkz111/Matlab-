...
%ALGORITHM_CONFIG GA / PSO / DE 三种传统优化算法统一配置
%
% 说明：
%   1. 保留 GA、PSO、DE 三种传统算法用于论文对比；
%   2. 默认采用普通电脑可承受的计算规模；
%   3. 输出进入带时间戳的新文件夹，避免 CSV 或图片被占用；
%   4. 默认后台保存图片，不弹出大量图窗；
%   5. 绘图颜色突出 PSO，便于论文中展示推荐方案。
...
% 默认计算规模：优先保证普通电脑能跑完。
% 需要更稳定的终稿结果时，可改为 nPop=28, maxIter=50, nRun=2 或 3。
cfg.common.nPop = 20;
